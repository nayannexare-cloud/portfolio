...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const closeModal = function () {
    ...
    // Remove 'open' first to clear its animation
    modal.classList.remove("open");
    // Use requestAnimationFrame to ensure browser re-paints before applying new animation
  ...
  const openModal = function (card) {
    ...
    // Ensure any in-progress closing animation is cleared
    modal.classList.remove("closing");
  ...
  // extra close button inside the drawer
  const drawerClose = document.querySelector('.drawer-close');
  if (drawerClose) {
    drawerClose.addEventListener('click', closeModal);
  }
  ...
  if (modal) {
    ...
    // Forward wheel events on the overlay to allow background scrolling
    ...
    // Touch forwarding for mobile: allow scrolling the page while overlay is present
    let _touchY = null;
    modal.addEventListener('touchstart', function (e) {
      if (e.target !== modal) return;
      _touchY = e.touches && e.touches[0] ? e.touches[0].clientY : null;
    }, { passive: true });

    modal.addEventListener('touchmove', function (e) {
      if (e.target !== modal || _touchY === null) return;
      e.preventDefault();
      const y = e.touches && e.touches[0] ? e.touches[0].clientY : _touchY;
      const delta = _touchY - y;
